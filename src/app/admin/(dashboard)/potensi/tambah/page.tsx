...
import { createPotensi } from '@/feature/admin/actions/potensi';
...
export default function TambahPotensiPage() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    formData.append("folder", "potensi");
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-montserrat-700 text-[#1C3F2D]">Tambah Potensi Desa</h2>
          <p className="text-[#414844] mt-1 font-inter-400 text-sm">Tambahkan data potensi desa baru.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6 md:p-8">
        <form action={createPotensi} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label htmlFor="title" className="block text-sm font-inter-600 text-gray-700 mb-2">Judul Potensi <span className="text-red-500">*</span></label>
              <input type="text" id="title" name="title" required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#285A43]" placeholder="Contoh: Sentra Industri Permebelan" />
            </div>

            <div>
              <label htmlFor="image" className="block text-sm font-inter-600 text-gray-700 mb-2">Gambar Potensi (Opsional)</label>
            ...
            <div>
              <label htmlFor="description" className="block text-sm font-inter-600 text-gray-700 mb-2">Deskripsi Potensi <span className="text-red-500">*</span></label>
              <textarea id="description" name="description" required rows={6} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#285A43] resize-y" placeholder="Jelaskan potensi desa ini..."></textarea>
            </div>
          </div>

          <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t border-gray-100">
            <Link href="/admin/potensi" className="w-full sm:w-auto text-center px-6 py-3 text-gray-600 font-inter-600 hover:bg-gray-100 rounded-xl transition">
              Batal
            </Link>
            <button type="submit" disabled={isUploading} className="w-full sm:w-auto text-center px-6 py-3 bg-[#0A2615] text-white font-inter-600 hover:bg-[#1C3F2D] rounded-xl transition shadow-sm disabled:opacity-70 disabled:cursor-not-allowed">
              {isUploading ? 'Tunggu Upload...' : 'Simpan Potensi'}
            </button>
          </div>
        </form>
      </div>
