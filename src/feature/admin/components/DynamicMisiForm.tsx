...
export default function DynamicMisiForm({ initialData, saveAction }: { initialData: string | undefined, saveAction: (formData: FormData) => Promise<void> }) {
  // Parse data JSON dari database
  const [items, setItems] = useState<MisiItem[]>(() => {
  ...
  const confirmDelete = () => {
    ...
    startTransition(async () => {
      try {
        ...
        formData.append('desa_misi', JSON.stringify(newItems));
  ...
  const updateItem = (index: number, field: keyof MisiItem, value: string) => {
    const newItems = [...items];
  ...
  return (
    <div className="space-y-4 relative">
          ...
          <div key={index} className="p-4 border border-gray-200 rounded-xl bg-gray-50/30 relative group">
            ...
            <div className="flex items-center gap-3 mb-3 pr-24">
              ...
              <input 
                ...
                placeholder="Judul Singkat Misi (Maks 3-4 kata)" 
            ...
            <div className="pl-9 space-y-3">
              <input 
                ...
                placeholder="URL Ikon (contoh: /assets/icons/leaf.svg)" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0A2615]/40 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 transform transition-all text-center">
            ...
            <p className="text-gray-500 font-inter-400 text-sm mb-6">
              Misi "{items[deleteIndex]?.title || 'ini'}" akan dihapus secara permanen dari database.
            </p>
      ...
      <input type="hidden" name="desa_misi" value={JSON.stringify(items)} />
    </div>
  );
}
