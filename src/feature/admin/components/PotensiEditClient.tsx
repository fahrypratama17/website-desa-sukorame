...
import { updatePotensi } from '@/feature/admin/actions/potensi';
import { uploadImage } from '@/feature/admin/actions/upload';
import type { Potensi } from '@prisma/client';

export default function PotensiEditClient({ potensi }: { potensi: Potensi }) {
  const [imageUrl, setImageUrl] = useState(potensi.image || '');
  const [isUploading, setIsUploading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Bind the ID to the server action
  const updatePotensiWithId = updatePotensi.bind(null, potensi.id);
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sm:p-6 md:p-8">
        <form action={updatePotensiWithId} className="space-y-6">
          <div className="space-y-4">
            <div>
              ...
              <input type="text" id="title" name="title" defaultValue={potensi.title} required className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#285A43]" />
            </div>
            ...
            <div>
              ...
              <textarea id="description" name="description" defaultValue={potensi.description} required rows={6} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#285A43] resize-y"></textarea>
            </div>
          </div>
